/*
  ...
  From Emily Lam, Sept 2018, Updated Aug 2019
...
static void test_alpha_display() {
    ...
    //if(ret == ESP_OK){ displayfunc();}
    char str[4]; //store user input

    uint16_t displaybuffer[128];


    //printf("%u\n",alphafonttable[122]);
    printf("%u\n",alphafonttable[(uint16_t)str[0]]); //I printed this stuff out to debug but you don't need it
    printf("%u\n",alphafonttable[(uint16_t)str[2]]);
    fflush(stdin);

    // Continually writes the same command
    while (1) {
      if(ret == ESP_OK) {
        printf("Enter a word: "); //move all of this stuff into a while loop to finish skill
        scanf("%s",str); //store user input into str
